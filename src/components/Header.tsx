        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex items-center gap-1">
              <button onClick={onOpenStudio} className="px-3 py-1.5 text-sm text-gray-400 hover:text-white transition-colors rounded-lg hover:bg-white/5">
                {t.nav.studio}
              </button>
              <button onClick={onOpenStudio} className="px-3 py-1.5 text-sm text-gray-400 hover:text-white transition-colors rounded-lg hover:bg-white/5">
                {t.nav.library}
              </button>
              <button className="px-3 py-1.5 text-sm text-gray-400 hover:text-white transition-colors rounded-lg hover:bg-white/5">
                {t.nav.pricing}
              </button>
              <button className="px-3 py-1.5 text-sm text-gray-400 hover:text-white transition-colors rounded-lg hover:bg-white/5">
                {t.nav.docs}
                ...
                {(['en', 'ru'] as Lang[]).map((l) => (
                  <button
                    ...
                    className={`px-2 py-0.5 text-xs font-medium rounded transition-all ${
                      lang === l ? 'bg-cyan-500 text-white shadow-sm' : 'text-gray-400 hover:text-white'
                    }`}
                  >
                    {l.toUpperCase()}
                  </button>
              ...
              {user ? (
                ...
                <>
                  <button
                    ...
                    className="hidden md:block px-3 py-1.5 text-sm text-gray-300 hover:text-white transition-colors"
                  >
                    {t.nav.signIn}
                  </button>
                  <button
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-white/5 bg-gray-950/95 backdrop-blur-md">
            <div className="px-4 py-3 flex flex-col gap-1">
              <button onClick={() => { onOpenStudio(); setMenuOpen(false); }} className="text-left px-3 py-2 text-sm text-gray-300 hover:text-white hover:bg-white/5 rounded-lg">
                {t.nav.studio}
              </button>
              <button onClick={() => { onOpenStudio(); setMenuOpen(false); }} className="text-left px-3 py-2 text-sm text-gray-300 hover:text-white hover:bg-white/5 rounded-lg">
                {t.nav.library}
              </button>
              <button className="text-left px-3 py-2 text-sm text-gray-300 hover:text-white hover:bg-white/5 rounded-lg">
                {t.nav.pricing}
              </button>
              <button className="text-left px-3 py-2 text-sm text-gray-300 hover:text-white hover:bg-white/5 rounded-lg">
                {t.nav.docs}
              ...
              <div className="flex gap-2 pt-2 border-t border-white/5 mt-1">
                {user ? (
                  <button onClick={handleSignOut} className="flex-1 px-3 py-2 text-sm text-red-400 border border-red-500/20 rounded-lg">
                    Sign out
                  </button>
                ) : (
                  <>
                    <button onClick={() => { setAuthModal('login'); setMenuOpen(false); }} className="flex-1 px-3 py-2 text-sm text-gray-300 border border-white/10 rounded-lg">
                      {t.nav.signIn}
                    </button>
                    <button onClick={() => { setAuthModal('register'); setMenuOpen(false); }} className="flex-1 px-3 py-2 text-sm font-medium bg-cyan-500 text-white rounded-lg">
                      {t.nav.signUp}
                    </button>
                  </>
                )}
