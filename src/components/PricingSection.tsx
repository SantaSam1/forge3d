...
export default function PricingSection({ onOpenStudio }: Props) {
  const { t } = useLang();
  ...
  const plans = [
    {
      ...t.pricing.free,
      highlight: false,
      action: () => onOpenStudio ? onOpenStudio() : navigate('/studio'),
    },
    {
      ...t.pricing.pro,
      highlight: true,
      action: () => window.open('https://t.me/pro3d_print_bot?start=pro', '_blank'),
    },
    {
      ...t.pricing.team,
      highlight: false,
      action: () => window.open('https://t.me/pro3d_print_bot?start=team', '_blank'),
    },
  ];
