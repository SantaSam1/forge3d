...
import {
  Sparkles, Upload, BookOpen, Download, Link2, X,
  ArrowLeft, Cpu, Layers, Search, Check, Clock, AlertCircle, History, ChevronDown
} from 'lucide-react';
...
export default function Studio({ onClose, addToast }: StudioProps) {
  ...
  // Mobile: show panel or viewer
  const [mobileView, setMobileView] = useState<'panel' | 'viewer'>('panel');
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      ...
      setUserId(uid);
      const query = supabase.from('models').select('id', { count: 'exact', head: true }).eq('source_type', 'ai_generated');
      if (uid) query.eq('user_id', uid); else query.is('user_id', null);
  ...
  const tabs: { id: Tab; label: string; icon: React.ComponentType<{ className?: string }> }[] = [
    ...
    { id: 'history', label: lang === 'ru' ? 'История' : 'History', icon: History },
  ];

  const handleGenerate = async () => {
    if (!prompt.trim()) return;
    if (generationCount >= FREE_LIMIT) {
      addToast(lang === 'ru' ? `Лимит исчерпан (${FREE_LIMIT}). Перейдите на Pro.` : `Free limit reached (${FREE_LIMIT}). Upgrade to Pro.`, 'error');
      return;
    }
    setGenerating(true);
    addToast(lang === 'ru' ? 'Запуск генерации...' : 'Starting generation...', 'info');
    try {
      ...
      if (res.status === 429) { addToast(lang === 'ru' ? 'Лимит исчерпан. Перейдите на Pro.' : 'Free limit reached. Upgrade to Pro.', 'error'); return; }
      if (!res.ok || data.error) throw new Error(data.error || 'Failed to start generation');
      const { task_id, model_id } = data;
      addToast(lang === 'ru' ? 'Генерация 3D-модели... (~1 мин)' : 'Generating 3D model... (~1 min)', 'info');
      let attempts = 0;
      const poll = async (): Promise<void> => {
        ...
        if (statusData.status === 'success') {
          ...
          setMobileView('viewer'); // switch to viewer on mobile after generation
          addToast(lang === 'ru' ? '3D-модель готова!' : '3D model ready!', 'success');
          return;
        }
  ...
  const loadUserModels = useCallback(async (force = false) => {
    ...
    const query = supabase.from('models').select('*').eq('source_type', 'ai_generated').order('created_at', { ascending: false }).limit(20);
    if (userId) query.eq('user_id', userId); else query.is('user_id', null);
  ...
  const handleHistoryLoad = (m: Model) => {
    if (m.file_url) {
      ...
      setActiveTab('generate'); setMobileView('viewer');
  ...
  return (
    <div className="fixed inset-0 z-40 bg-gray-950 flex flex-col">
      {/* Top bar */}
      <div className="flex items-center justify-between px-3 sm:px-4 py-3 border-b border-white/5 flex-shrink-0">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          <button onClick={onClose} className="flex items-center gap-1.5 text-gray-400 hover:text-white transition-colors text-sm flex-shrink-0">
            <ArrowLeft className="w-4 h-4" />
          </button>
          <div className="h-4 w-px bg-white/10 flex-shrink-0" />
          <div className="flex items-center gap-2 min-w-0">
            <Cpu className="w-4 h-4 text-cyan-400 flex-shrink-0" />
            <span className="text-white font-medium text-sm flex-shrink-0">{t.studio.title}</span>
            {modelName && <span className="text-gray-500 text-sm truncate hidden sm:block">/ {modelName}</span>}
          </div>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          {/* Mobile toggle */}
          <button
            onClick={() => setMobileView(mobileView === 'panel' ? 'viewer' : 'panel')}
            className="sm:hidden px-2.5 py-1 bg-white/5 border border-white/10 rounded-lg text-xs text-gray-400"
          >
            {mobileView === 'panel' ? (lang === 'ru' ? 'Вид' : 'View') : (lang === 'ru' ? 'Панель' : 'Panel')}
          </button>
          <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 bg-white/5 rounded-lg border border-white/10">
            ...
            <span className="text-xs text-gray-400">
              <span className={remaining === 0 ? 'text-red-400 font-medium' : 'text-white font-medium'}>{remaining}</span>/{FREE_LIMIT}
            </span>
          </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Left panel — full width on mobile when panel view, hidden when viewer view */}
        <div className={`
          flex-shrink-0 border-r border-white/5 flex flex-col bg-gray-950
          ${mobileView === 'viewer' ? 'hidden sm:flex sm:w-80' : 'flex w-full sm:w-80'}
        `}>
          {/* Tabs */}
            ...
            {tabs.map(({ id, label, icon: Icon }) => (
              <button key={id} onClick={() => handleTabChange(id)}
                className={`flex-1 flex flex-col items-center gap-1 py-3 text-xs transition-all ${
                  activeTab === id ? 'text-cyan-400 border-b-2 border-cyan-500' : 'text-gray-500 hover:text-gray-300'
                }`}>
              ...
              <div className="flex flex-col gap-4">
                {remaining === 0 && (
                  <div className="flex items-start gap-2 p-3 bg-red-500/10 border border-red-500/20 rounded-xl">
                    ...
                    <div>
                      <p className="text-xs font-medium text-red-400">{lang === 'ru' ? 'Лимит исчерпан' : 'Free limit reached'}</p>
                      <p className="text-xs text-gray-500 mt-0.5">{lang === 'ru' ? `Использованы все ${FREE_LIMIT} генераций. Перейдите на Pro.` : `You've used all ${FREE_LIMIT} free generations. Upgrade to Pro.`}</p>
                    </div>
                  </div>
                )}
                {remaining > 0 && remaining <= 2 && (
                  <div className="flex items-center gap-2 p-2.5 bg-yellow-500/10 border border-yellow-500/20 rounded-xl">
                    ...
                    <p className="text-xs text-yellow-300">{lang === 'ru' ? `Осталось ${remaining} генерации` : `${remaining} generation${remaining !== 1 ? 's' : ''} remaining`}</p>
                  </div>
                )}
                <div>
                  <label className="text-xs font-medium text-gray-400 block mb-2">{t.studio.generate.label}</label>
                  <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)}
                    placeholder={t.studio.generate.placeholder} rows={4}
                ...
                <div>
                  <p className="text-xs text-gray-600 mb-2">{lang === 'ru' ? 'Быстрые промпты:' : 'Quick prompts:'}</p>
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-2">
                  {filteredLibrary.map((item) => (
                    <div key={item.id} onClick={() => { if (item.file_url) { setModelUrl(item.file_url); setModelFormat('glb'); setModelName(item.name); setMobileView('viewer'); } }}
                  ...
                  {filteredLibrary.length === 0 && <p className="text-center text-gray-600 text-sm py-8">{lang === 'ru' ? 'Модели не найдены' : 'No models found'}</p>}
                </div>
              </div>
            )}
            ...
            {activeTab === 'history' && (
              <div className="flex flex-col gap-3">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-xs font-medium text-gray-400">{lang === 'ru' ? 'Ваши генерации' : 'Your generations'}</p>
                  <button onClick={() => loadUserModels(true)} className="text-xs text-cyan-500 hover:text-cyan-400 transition-colors">
                    {lang === 'ru' ? 'Обновить' : 'Refresh'}
                  </button>
                </div>
                {!modelsLoaded ? (
                  <div className="flex justify-center py-8"><div className="w-5 h-5 border-2 border-cyan-500/30 border-t-cyan-500 rounded-full animate-spin" /></div>
                ) : userModels.length === 0 ? (
                  <div className="text-center py-8">
                    <p className="text-gray-600 text-sm">{lang === 'ru' ? 'Нет генераций' : 'No generations yet'}</p>
                    <p className="text-gray-700 text-xs mt-1">{lang === 'ru' ? 'Создайте первую 3D-модель выше' : 'Generate your first 3D model above'}</p>
                  </div>
                ) : (
                  <div className="flex flex-col gap-2">
                    {userModels.map((m) => (
                      <div key={m.id}
                        className={`p-3 bg-gray-900/50 rounded-xl border border-white/5 transition-all ${m.status === 'ready' ? 'hover:border-white/10 hover:bg-gray-900 cursor-pointer' : ''}`}
                        onClick={() => m.status === 'ready' && handleHistoryLoad(m)}>
                        <div className="flex items-start gap-2">
                          <div className="flex-1 min-w-0">
                            <p className="text-xs text-gray-300 truncate font-medium">{m.name}</p>
                            {m.prompt && <p className="text-xs text-gray-600 mt-0.5 truncate">{m.prompt}</p>}
                            <p className="text-xs text-gray-700 mt-1">{new Date(m.created_at).toLocaleDateString()}</p>
                          </div>
                          <div className="flex items-center gap-1 flex-shrink-0 mt-0.5">
                            {statusIcon(m.status)}
                            <span className={`text-xs ${m.status === 'ready' ? 'text-green-400' : m.status === 'processing' ? 'text-yellow-400' : m.status === 'failed' ? 'text-red-400' : 'text-gray-500'}`}>
                              {m.status === 'ready' ? (lang === 'ru' ? 'готово' : 'ready') : m.status === 'processing' ? (lang === 'ru' ? 'обработка' : 'processing') : m.status === 'failed' ? (lang === 'ru' ? 'ошибка' : 'failed') : m.status}
                            </span>
                          </div>
                        </div>
                      </div>
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {EXPORT_FORMATS.map((f) => (
                  <button key={f} onClick={() => setExportFormat(f)}
                    className={`px-2.5 py-1 text-xs rounded-lg transition-all ${exportFormat === f ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'bg-gray-800 text-gray-500 hover:text-gray-300 border border-white/5'}`}>
                    {f.toUpperCase()}
                  </button>
                ))}
              </div>
        ...
        {/* Viewer area — full width on mobile when viewer view */}
        <div className={`flex-1 relative ${mobileView === 'panel' ? 'hidden sm:block' : 'block'}`}>
